...
public class UIMgrEditorWindow : EditorWindow
{
    ...
    private void DrawStatistics(UIMgr uiMgr)
    {
        ...
        EditorGUILayout.LabelField("状态: " + (openedCount > 0 ? "<color=green>激活</color>" : "<color=gray>未激活</color>"), _statusLabelStyle);
    ...
    private void DrawRegisteredPanels(UIMgr uiMgr)
    {
        ...
        foreach (var kvp in configs)
        {
            ...
            EditorGUILayout.LabelField($"多个: {GetBoolDisplayText(config.allowMultiple)}", GUILayout.Width(80));
