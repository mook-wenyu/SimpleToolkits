...
namespace SimpleToolkits
{
    /// <summary>
    /// 布局自动尺寸提供器基类
    /// 提供基于Unity布局组件（HorizontalOrVerticalLayoutGroup、ContentSizeFitter、LayoutElement）的自动尺寸计算
    /// </summary>
    public abstract class AutoSizeProvider : IVariableSizeAdapter
    {
        ...
        protected AutoSizeProvider(
            ...
            bool useLayoutGroups = true,
            bool forceRebuild = false)
            : this(template, new Vector2(fixedWidth, -1f), new Vector2(fixedWidth, minHeight), new Vector2(fixedWidth, maxHeight), useLayoutGroups, forceRebuild)
        {
        }
        #endregion
        ...
        protected virtual Vector2 GetLayoutSize(int index, Vector2 viewportSize, IScrollLayout layout)
        {
            ...
            {
                Debug.LogWarning($"[AutoSizeProvider] 布局计算失败: {e.Message}");
                return Vector2.zero;
            }
        }
        ...
        private void RebuildLayout()
        {
            ...
            {
                Debug.LogWarning($"[AutoSizeProvider] 布局重建失败: {e.Message}");
            }
        }
